...
WITH t_dg AS (
  SELECT *
  FROM
    `physionet-data.eicu_crd.patient` t1
  WHERE
    t1.patientunitstayid in (
      SELECT
        tin_1.patientunitstayid
      FROM
      `physionet-data.eicu_crd.diagnosis` tin_1
      WHERE diagnosispriority = 'Major'
      AND(
    ...
    AND t1.patientunitstayid NOT in (
      SELECT tex_1.patientunitstayid
      FROM `physionet-data.eicu_crd.diagnosis` tex_1
      WHERE diagnosispriority = 'Major'
      AND(
...
SELECT *
FROM t_dg
LEFT JOIN `physionet-data.eicu_crd.apachepatientresult`
